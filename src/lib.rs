...
impl CompactSize {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let prefix = bytes[0];
        match prefix {
            0..=252 => Ok((CompactSize::new(prefix as u64), 1)),
            0xFD => {
                if bytes.len() < 3 {
                    return Err(BitcoinError::InsufficientBytes);
                }
...
impl OutPoint {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        txid_array.copy_from_slice(&bytes[0..32]);
...
impl Script {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let length_prefix = CompactSize::new(result.len() as u64).to_bytes();
        [length_prefix, result].concat()
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let script_bytes = bytes[prefix_size..total_size].to_vec();
        Ok((
            Script {
                bytes: script_bytes,
            },
...
impl fmt::Display for BitcoinTransaction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for (i, input) in self.inputs.iter().enumerate() {
            ...
            // FIX: test only expects vout, not txid
            writeln!(
                f,
